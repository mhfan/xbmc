#!/bin/bash
 ################################################################
 # $ID: ffmpeg.sh      Èý, 28  7ÔÂ 2010 10:53:13 +0800  mhfan $ #
 #                                                              #
 # Description:                                                 #
 #                                                              #
...
[ $# -lt 1 ] && echo "Usage: $0 /path/to/media.file [/path/to/audio.file]" \
	     && exit 1
...
if [ -n "$2" ]; then AF=$2; else
      if  [ -f $MF.wav ]; then AF=$MF.wav
    elif  [ -f $MF.WAV ]; then AF=$MF.WAV
    else AF=; fi
fi
...
#LD_LIBRARY_PATH=debian-shared/libavutil:debian-shared/libavcodec:debian-shared/libavformat:debian-shared/libavcore:debian-shared/libavdevice:debian-shared/libswscale:debian-shared/libpostproc:debian-shared/libavfilter debian-shared/

ffmpeg  -i $1 ${2:+-i $2} ${3:+-i $3} \
	-vcodec mpeg2video -acodec mp2 -sameq -aq 99 -ab 128000 \
	-timestamp now -metadata comment="Transformed by M.H.Fan" \
	${1%.*}.mpg ${3:+-acodec mp2 -newaudio} \

#-b 6000000 -maxrate 9000000 -bufsize 1835008 \
#-shortest -dpf 200

#ffplay -ast 0 /path/to/media.file
...
if false; then
  ...
  http://www.cnitblog.com/dvb-dvb/archive/2009/04/02/55951.html
