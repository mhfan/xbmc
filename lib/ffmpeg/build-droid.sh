...
if true; then
ARCH=${ARCH:-$(uname -m)};
PREFIX=/usr; CROSS_COMPILE=; #CPU=core2;
else	# XXX:
export ANDROID_NDK=/home/mhfan/devel/mhdroid/ndk
ARCH=armdroid; PREFIX=${PWD}/extras/contrib/hosts/arm-eabi
...
$SRCDIR/configure \
  ...
  --enable-filters \
    --enable-filter="crop" \
  --disable-devices \
    --disable-indev="alsa,v4l2" \
    --disable-outdev="alsa,v4l2" \
  \
  ...
  $@ \
  \
&& sed -i -e 's/\(.*CONFIG_SMALL.*\)0/\1 1/' config.h \
&& sed -i -e 's/\(.*\(ARCH_ARM\).*\)/#undef \2\n\1/' config.h \
&& sed -i -e 's/\(.*\(HAVE_PTHREADS\).*\)/#undef \2\n\1/' config.h \
#&& touch configure-stamp \

#&& sed -i -e 's/\(^HAVE_ARMV.*\)/#\1/' config.mak \
#&& sed -i -e 's/\(^HAVE_NEON.*\)/#\1/' config.mak \
#&& sed -i -e 's/\(.*HAVE_ARMV.*\)1/\1 0/' config.h \
#&& sed -i -e 's/\(.*HAVE_NEON.*\)1/\1 0/' config.h \
#&& sed -i -e 's/\(.*HAVE_LOG2.*\)1/\1 0/' config.h \
#&& sed -i -e 's/\(.*POSIX_MEMALIGN.*\)1/\1 0/' config.h \
